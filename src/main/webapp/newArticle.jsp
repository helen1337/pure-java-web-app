...
<body>
<div class="body">
    <form action="/my-blog/article" method="post" class="needs-validation" novalidate="">
        ...
        <div class="containerBody">
            ...
            <div class="col-md-8 mx-auto">
                ...
                <input type="title" name="title" placeholder="Title" class="form-control" id="title"  required=""
                <c:choose>
                ...
                <input type="text" name="author" placeholder="Enter author" id="author" class="form-control" required=""
                       value="<c:choose><c:when test="${not empty article}">${article.author}</c:when><c:otherwise><c:if test="${not empty sessionScope.user}">${sessionScope.user.login}</c:if></c:otherwise></c:choose>">
                <div class="invalid-feedback">
                ...
                <input type="text" name="theme" placeholder="Enter theme" id="theme" class="form-control" required=""
                       value="<c:if test="${not empty article}">${article.theme}</c:if>">
                <div class="invalid-feedback">
                ...
                <textarea type="full_text" name="content"  class="form-control" placeholder="Enter the full text of the article" id="content" required=""
                          value=""
                          style="height: 100px"><c:if test="${not empty article}">${article.content}</c:if></textarea>
                <div class="invalid-feedback">
